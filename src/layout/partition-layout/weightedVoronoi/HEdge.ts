...
export class HEdge {
  ...
  isHorizon() {
    let horizonal =
      this.twin !== null && !this.iFace.singed && this.twin.iFace.singed;
    return horizonal;
  }
  ...
  isEqual(source: any, target: any): boolean {
    let startSame: boolean =
      this.origin.equals(source) && this.destine.equals(target);
    let endSame: boolean =
      this.origin.equals(target) && this.destine.equals(source);
