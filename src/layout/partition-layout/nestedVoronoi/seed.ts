...
const seed = (seedNum: number): any => {
  ...
  const pseudoRandomNumGenerator = (): number => {
    ...
    while (number < significantBit) {
      ({ number, denominator, significant } = dealUnderflow(
        number,
        denominator,
        significant,
        RC4
    ...
    while (number >= significantBit * 2) {
      ({ number, denominator, significant } = dealOverflow(
        number,
        denominator,
        significant
      ));
...
const rc4Encrypt = (
  ...
  for (loalI = 0; loalI < rc4ModUnit; loalI++) {
    let biter =
      localJ + keyStream[mod(loalI, keyStream.length)] + localScheduling[loalI];
    localJ = bitMask & biter;
    // 交换
    [localScheduling[loalI], localScheduling[localJ]] = [
      localScheduling[localJ],
      localScheduling[loalI],
    ];
  }
  // 返回结果数
...
const charCodeToStr = (charCode: number[]): string =>
  String.fromCharCode.apply(this, charCode);
export { seed };
