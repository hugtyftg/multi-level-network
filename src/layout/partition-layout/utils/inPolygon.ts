// 点是否在多边形内（包含视觉效果的更严格计算）——方法2，如果点到每条边的距离都大于某个阈值，则认为它在多边形内
...
function isInPolygonVisually(
  excitationPadding: number | any,
  [currentCx, currentCy]: number[] | any,
  [maxIncircleCx, maxIncircleCy]: number[] | any,
  polygon: number[] | any
) {
