export class FlickeringMitigation {
  ...
  totalArea(newAllAbailbaleArea?: any) {
    ...
    if (parseFloat(newAllAbailbaleArea) > 0) {
      ...
      console.warn(
        'FlickeringMitigation.totalArea() 只接受正数 ' + newAllAbailbaleArea
      );
    }
    return this;
  }
  add(areaError: any) {
    ...
    if (!isNaN(secondTolastErrorOfArea)) {
      ...
      this.lastGrowth = direction(
        this.lastErrorOfArea,
        secondTolastErrorOfArea
      );
    }
  ...
  ratio() {
    ...
    if (this.lastErrorOfArea > this.allAvailableArea / 10) {
      return 0;
    }
