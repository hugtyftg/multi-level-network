...
export default class MultiLevelPartitionGraph extends BaseGraph {
  ...
  protected beforeRenderBindEvent() {
    ...
    const zoomObj = zoom()
      ...
      .on('zoom', (event: any) => {
        ...
        // 标签的自动隐藏和现实效果
        if (
          event.transform.k > (this.cfgs.scaleThreshold as number) &&
          this.nodesDOM
        ) {
          this.nodesDOM?.selectAll('text').attr('display', 'block');
        } else {
          this.nodesDOM
            ?.selectAll('text')
  ...
  private async loadDataAndDraw() {
    ...
    this.weightedHierarchicalData = hierarchy(hierarchicalData)
      ...
      .sum((d) =>
        Object.hasOwnProperty.call(d, 'num')
    ...
    // 计算指定图形的画布顶点坐标
    let canvasPolygon: any = calculateShapeCanvas(
      0,
      0,
      this._width,
      this._height,
    ...
    // 绘制Group和GroupLink
    this.drawBottom(
      this.partitionLayoutCell,
      this.allNodesData,
      this.groupIncircleMap,
      this.groupPolygonMap
    );
  }
  // 取消高亮
  private cancelHighlight() {
    ...
    selectAll('#bottom-node-cell text').attr('display', (d: any) =>
      nodeLabelDisplayFn(d, this.cfgs as StyleCfg)
    );
  }
  // highlightAttr是方便搜索ip的时候复用这个函数
  private highlightNodesEdges(
    d: any,
    clickedNodeGroupIndex: any,
    relatedNodeGourpIndexSet: any,
    highlightAttr?: {
      nodeStyle?: NodeStyle;
      edgeStyle?: EdgeStyle;
      maskStyle?: MaskStyle;
    ...
    // 当前节点是否和高亮节点相关
    const isRelateNode = (d: any): boolean =>
      relatedNodeGourpIndexSet.has(d['groupIndex']);
    // 解构以便复用
    ...
    selectAll('#bottom-edge-cell line')
      .attr('opacity', (d: any) => {
        if (isRelateEdge(d)) {
          ...
          return (
            highlightAttr?.edgeStyle?.opacity ??
            (edgeStyle.selected.opacity as number)
          );
        } else {
      ...
      .attr('stroke', (d: any) =>
        isRelateEdge(d)
          ? highlightAttr?.edgeStyle?.strokeColor ??
            edgeStyle.selected.strokeColor
          : edgeStyle.normal.strokeColor
      )
      .attr('stroke-width', (d: any) =>
        isRelateEdge(d)
          ? highlightAttr?.edgeStyle?.strokeWidth ??
            (edgeStyle.selected.strokeWidth as number)
          : (edgeStyle.normal.strokeWidth as number)
      )
      .attr('stroke-dasharray', (d: any) =>
        isRelateEdge(d)
          ? highlightAttr?.edgeStyle?.strokeDash ??
            edgeStyle.selected.strokeDash
          : edgeStyle.normal.strokeDash
      );
    // symbol node
    ...
    selectAll('#bottom-node-cell circle.circle-background')
      .attr('opacity', (d: any) =>
        isRelateNode(d)
          ? highlightAttr?.nodeStyle?.opacity ??
            (nodeStyle.selected.opacity as number)
          : (nodeStyle.normal.opacity as number)
      )
      .attr('stroke-width', (d: any) =>
        isRelateNode(d)
          ? highlightAttr?.nodeStyle?.strokeWidth ??
            (nodeStyle.selected.strokeWidth as number)
          : (nodeStyle.normal.strokeWidth as number)
      )
      .attr('stroke', (d: any) =>
        isRelateNode(d)
          ? highlightAttr?.nodeStyle?.stroke ?? nodeStyle.selected.stroke
          : nodeStyle.normal.stroke
      )
      .attr('fill', (d: any) => {
        if (isRelateNode(d)) {
          if (highlightAttr) {
            ...
            return typeof nodeStyle.selected.fill === 'string'
              ? nodeStyle.selected.fill
              : nodeStyle.selected.fill(d);
          }
        } else {
          return typeof nodeStyle.normal.fill === 'string'
            ? nodeStyle.normal.fill
            : nodeStyle.normal.fill(d);
        }
      });
  ...
  private highlightMask(
    ...
    this.container
      .selectAll(`path.pod[data-id=${relatedMask}]`)
      // 存在某些pod高亮改顺序之后，遮盖az的边界的问题
      // .raise()
      .attr(
        'opacity',
        highlightAttr?.maskStyle?.opacity ??
          this.cfgs.maskStyle?.selected.opacity
      )
      .attr(
        'fill',
        highlightAttr?.maskStyle?.color ?? this.cfgs.maskStyle?.selected.color
      );
  ...
  private highlight(
    ...
    // 将相关联的边及其节点高亮
    this.highlightNodesEdges(
      this.clickedNodeGroupDatum,
      this.clickedNodeGroupIndex,
      relatedNodeGourpIndexSet,
      highlightAttr
    );
    // 还原mask
    this.resetMask('pod');
    ...
    for (const relatedGroupIndex of relatedNodeGourpIndexSet) {
      // 关联到的点对象，里面有层级信息
      const relatedGroupDatum = allNodesData.find(
        (item: any) => item.groupIndex === relatedGroupIndex
      );
  ...
  private drawVoronoiDomain(weightedHierarchicalData: any) {
    ...
    // 渲染层级化voronoi多边形
    this.voronoiPathCell = renderVoronoiPath(
      this.partitionLayoutCell,
      this.cfgs,
  ...
  private drawShapeCanvas(
    canvasPolygon: number[][],
    nestedVoronoiCenter: number[]
  ) {
    // 绘制容器
    this.partitionLayoutCell = this.container
      .append('g')
  ...
  private partition(canvasPolygon: number[][], weightedHierarchicalData: any) {
    ...
    for (let i = 0; i < weightedHierarchicalData.children.length; i++) {
      centralizing(
        weightedHierarchicalData.children[i],
        'name',
        this.cfgs.emphasisName as string
      );
    }
    /* ------------------cnt中心化----------------- */
  }
  ...
  // 存储每一级的区域信息
  private saveRegionInfo(
    weightedHierarchicalData: any,
    min: any,
    max: any,
    allNodesData: any,
    groupIncircleMap: any,
    groupPolygonMap: any
  ) {
    if (weightedHierarchicalData.children[0].data.hierarchy === 'pod') {
      ...
      weightedHierarchicalData.children.forEach((d: any) => {
        ...
        let azPodName: string = `${d.data.name}${
          d.height === 0 ? '-' + d.parent.data.name : ''
        }`;
  ...
  private staggerRenderVoronoiLabel(fontScale: any, delayTime: number) {
    ...
    this.voronoiLabelCell = renderVoronoiLabel(
      fontScale,
      this.partitionLayoutCell,
      this.cfgs,
      this.hierarchicalInfo
  ...
  private partitionForceLayout(
    ...
    const drag = () => {
      ...
      const dragstarted = (event: any, nodeDatum: any) => {
        ...
        asSourceEdgeEl = selectAll(
          `g[sourceGroupIndex="${nodeDatum.groupIndex}"] line`
        );
        asTargetEdgeEl = selectAll(
          `g[targetGroupIndex="${nodeDatum.groupIndex}"] line`
        );
      };
      const dragged = (event: any, nodeDatum: any) => {
        ...
        /* ---------------------问题：多边形区域限制--------------------- */
        let constraintX = groupPolygonTickerMap[nodeDatum.group](
          5,
          1,
          event.y,
          event.x,
        ...
        let constraintY = groupPolygonTickerMap[nodeDatum.group](
          5,
          1,
          event.x,
          event.y,
        ...
        // drag的时候改变节点group的位置
        dragNodeEl.attr('transform', (d: any) => `translate(${d.x}, ${d.y})`);
    ...
    const getEdgesData = (allNodesData: any, originEdgesData: any) => {
      ...
      for (let i = 0; i < originEdgesData.length; i++) {
        ...
        let source = allNodesData.find(
          (d: any) => d.groupIndex === originEdgeSourceGroupIndex
        );
        let target = allNodesData.find(
          (d: any) => d.groupIndex === originEdgeTargetGroupIndex
        );
        allEdgesData.push({
          source,
          target,
    ...
    const onTickEnd = () => {
      ...
      // 渲染点边
      this.edgesDOM = renderEdgesElWithoutTick(
        allEdgesData,
        this.bottomEdgeCell,
        this.cfgs
      );
      this.nodesDOM = renderNodesElWithoutTick(
        allNodesData,
        this.container,
        this.bottomNodeCell,
        drag,
    ...
    let simulation = forceSimulation(allNodesData)
      ...
      .force(
        'collide',
        forceCollide()
          .radius(this.cfgs.blankFillDegree as number)
          .strength(this.cfgs.blankFillStrength as number)
      )
      .alphaDecay(0.15);
  ...
  public searchPartition(
    level2Name: string,
    level3Name: string,
    isHighlightNodes: boolean,
    isHighlightEdges: boolean,
    highlightAttr?: {
      nodeStyle?: NodeStyle;
      edgeStyle?: EdgeStyle;
      maskStyle?: MaskStyle;
    ...
    const searchedPartition = this.hierarchicalInfo.find(
      (item: any) =>
        item.data.name === level3Name && item.parent.data.name === level2Name
    );
    ...
    this.container
      .selectAll(`path.pod[data-id=${searchedPartitionId}]`)
      // .raise()
      .attr(
        'opacity',
        highlightAttr?.maskStyle?.opacity ??
          this.cfgs.maskStyle?.selected.opacity
      ) // this.cfgs.maskStyle.selected.opacity和dii中的d.attr.selected.opacity是一样的逻辑，全局样式参数
      .attr(
        'fill',
        highlightAttr?.maskStyle?.color ?? this.cfgs.maskStyle?.selected.color
      );
    ...
    if (isHighlightNodes) {
      if (this.nodesDOM) {
        ...
        const innerNodesSelection = this.nodesDOM.filter((node: any) => {
          ...
          return (
            node.level3Id === searchedPartitionId &&
            innerGroupIndexList.push(node.groupIndex)
          );
        });
        // 更新selection的样式——节点的背景（circle-background是仿照dii写的）
        innerNodesSelection
          .selectAll('circle.circle-background')
          .attr(
            'opacity',
            highlightAttr?.nodeStyle?.opacity ??
              this.cfgs.nodeStyle.selected.opacity
          )
          .attr(
            'stroke-width',
            highlightAttr?.nodeStyle?.strokeWidth ??
              this.cfgs.nodeStyle.selected.strokeWidth
          )
          .attr(
            'stroke',
            highlightAttr?.nodeStyle?.stroke ??
              this.cfgs.nodeStyle.selected.stroke
          )
          // 判断用户是否传入新的样式fill，如果没有传入，就用默认的样式，并且默认的fill可能是string，也可能是function
          .attr(
            'fill',
            (d: any) =>
              highlightAttr?.nodeStyle?.fill ??
              // 这里是判断初始传入的配置项是函数还是字符串
              (typeof this.cfgs.nodeStyle.normal.fill === 'string'
                ? this.cfgs.nodeStyle.normal.fill
                : this.cfgs.nodeStyle.normal.fill(d))
          );
    ...
    if (isHighlightEdges) {
      if (this.edgesDOM) {
        ...
        const innerEdgesSelection = this.edgesDOM.filter((edge: any) => {
          return (
            innerGroupIndexList.findIndex(
              (groupIndex: number) => groupIndex === edge.source.groupIndex
            ) !== -1 &&
            innerGroupIndexList.findIndex(
              (groupIndex: number) => groupIndex === edge.target.groupIndex
            ) !== -1
          );
        });
        // 更新selection的样式——连边
        ...
        innerEdgesSelection
          .attr(
            'opacity',
            highlightAttr?.edgeStyle?.opacity ??
              this.cfgs.edgeStyle.selected.opacity
          )
          .attr(
            'stroke-width',
            highlightAttr?.edgeStyle?.strokeWidth ??
              this.cfgs.edgeStyle.selected.strokeWidth
          )
          .attr(
            'stroke',
            highlightAttr?.edgeStyle?.strokeColor ??
              this.cfgs.edgeStyle.selected.strokeColor
          )
          .attr(
            'stroke-dasharray',
            highlightAttr?.edgeStyle?.strokeDash ??
              this.cfgs.edgeStyle.selected.strokeDash
          );
      } else {
  ...
  public searchIp(
    ...
    if (this.nodesDOM && this.edgesDOM && this.voronoiPathCell) {
      ...
      const searchedNodeDatum = this.allNodesData.find(
        (node: any) => !node.isHyperNode && node.children[0].mgmt_ip === ip
      );
